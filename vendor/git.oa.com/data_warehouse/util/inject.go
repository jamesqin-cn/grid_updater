...
func newCommonProvider(names map[string]reflect.Type) Provider {
	return ProviderFunc(func() []*inject.Object {
		...
		for name, t := range names {
			...
			case name == "db":
				v = NewMySQLWithConfigSvc("database", false)
			case strings.HasPrefix(name, "db."):
				v = NewMySQLWithConfigSvc(strings.TrimPrefix(name, "db."), false)
			case strings.HasPrefix(name, "cfg."):
				v = reflect.New(t.Elem()).Interface()
				configName := strings.TrimPrefix(name, "cfg.")
				LoadConfig(configName, v)
			}
			if v != nil {
